...
email.addEventListener("input", () => {
    if (email.value && !checkEmail(email.value)){
        createDisplayMsgError("O Email não é valido");
    }else{
        createDisplayMsgError("");
    }
})
...
function checkPasswordStrength(senha) {
    if (!/[a-z]/.test(senha)) {
        return "A senha daeve ter pelo menos uma letra minúscula!";
    }
    if (!/[A-Z]/.test(senha)) {
        return " A senha deve ter pelo menos uma letramaiúscula!";
    }
    if (!/[\w_]/.test(senha)) {
...
const maskCpf = (event) => {
    let cpfValor = event.target.value

    cpfValor = cpfValor.replace(/\D/g, ""); // Remove os caracteres não numéricos
    
    if (cpfValor.length > 11) {
        cpfValor = cpfValor.substring(0, 11);
    }

    console.log(cpfValor.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/g,"\$1.\$2.\$3\-\$4"))

    event.target.value = cpfValor.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/g,"\$1.\$2.\$3\-\$4");
    }
...
const maskRG = (event) => {
    ...
    console.log(RG.replace(/(\d{2})(\d{3})(\d{3})(\d{1})/g,"\$1.\$2.\$3\-\$4"))

    event.target.value = RG.replace(/(\d{2})(\d{3})(\d{3})(\d{1})/g,"\$1.\$2.\$3\-\$4");
    }
